...
void ConnectToEngine(const char* path)
{
	...
	CreateProcess(NULL, (char*)path, NULL, NULL, TRUE, 0, NULL, NULL, &sti, &pi);
}

std::string getNextMove(std::string &position)
{
	std::string str;
	std::string request = "position startpos moves " + position + "\ngo\n";
	WriteFile(pipin_w, request.c_str(), request.length(), &writ, NULL);
	Sleep(1000);
	std::string stop = "stop\n";
	WriteFile(pipin_w, stop.c_str(), stop.length(), &writ, NULL);
	Sleep(500);
